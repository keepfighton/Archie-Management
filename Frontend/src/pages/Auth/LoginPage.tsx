    ...
    <div className="flex min-h-screen flex-col bg-slate-100 lg:flex-row">
      ...
      <div className="flex flex-col min-h-screen lg:hidden relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col items-center pt-12 pb-4 px-6">
          <img src={nexoraPartUrl} alt="Nexora" className="h-14 w-auto object-contain drop-shadow-lg" />
          <div className="mt-4 text-center">
            <h1 className="text-xl font-bold text-white tracking-tight">Business. Simplified.</h1>
            <p className="mt-1 text-xs text-blue-200/70">Your integrated workspace platform</p>
          </div>
          <div className="mt-3 flex flex-wrap justify-center gap-1.5">
            {['Finance', 'CRM', 'Operations', 'Team'].map(f => (
              <span key={f} className="px-2.5 py-0.5 rounded-full text-[10px] font-medium text-blue-100/80"
        ...
        <div className="relative z-10 mx-4 rounded-xl overflow-hidden flex-1 min-h-0"
             style={{ boxShadow: '0 8px 32px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <img
            src={frameKananUrl}
            alt="NEXONE Dashboard"
            className="w-full h-full object-cover object-left-top"
          />
          {/* subtle bottom fade so it blends into the form card */}
          <div className="absolute bottom-0 left-0 right-0 h-16"
               style={{ background: 'linear-gradient(to bottom, transparent, rgba(10,26,62,0.9))' }} />
        </div>

        {/* ── Form card ── */}
        <div className="relative z-10 mx-4 mb-4 mt-3 rounded-2xl p-5 shadow-2xl"
             style={{ background: 'rgba(255,255,255,0.97)' }}>

          <h2 className="text-xl font-bold text-gray-900 leading-tight mb-0.5">Welcome back</h2>
          <p className="text-[12px] text-gray-400 mb-4">Sign in to access your workspace</p>

          {error && (
            <div className="mb-3 px-4 py-2.5 rounded-xl text-sm text-red-600 bg-red-50 border border-red-200">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} autoComplete="off" className="space-y-3">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #0d1f3c 0%, #1a3a6b 100%)',
                       boxShadow: '0 6px 20px rgba(13,31,60,0.35)' }}
            >
              {loading
