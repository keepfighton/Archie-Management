    ...
    <div className="p-5">
      ...
      >
        <div className="space-y-4">
          ...
          <FormField label={t('internalProjects.name', 'Project name')} required><input autoFocus className="input" value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder={t('internalProjects.namePlaceholder', 'Example: Archie Management Product Development')} /></FormField>
      ...
      >
        {memberProject && canManage(memberProject) && <div className="mb-4 flex flex-col gap-2 rounded-2xl border border-gray-200 bg-slate-50 p-3 sm:flex-row">
          <div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3 text-gray-400" size={15} /><select className="input pl-9" value={selectedUserId} onChange={event => setSelectedUserId(event.target.value)}><option value="">{t('internalProjects.selectMember', 'Select a registered Archie Management user...')}</option>{availableUsers.map(user => <option key={user.id} value={user.id}>{user.name} - {user.email}</option>)}</select></div>
